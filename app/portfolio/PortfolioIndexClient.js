...
import { portfolioIndex } from '@/dictionaries/portfolio';
...
export default function PortfolioIndexClient() {
  ...
  const t = { ...common[lang], ...portfolioIndex[lang] };
  const portfolio = PORTFOLIO[lang];
  const liveCount = portfolio.filter((item) => item.url).length;
    ...
    <>
      ...
      <section className="border-b border-border bg-[linear-gradient(180deg,#F5F8F6,#FBFCFB)]">
        <div className="max-w-[1100px] mx-auto px-[22px] pt-[clamp(28px,4vw,44px)] pb-[clamp(42px,6vw,70px)]">
          <Breadcrumb items={[{ label: t.crumbHome, href: '/' }]} current={t.crumbPortfolio} />
          <div className="animate-reveal motion-reduce:animate-none mt-6 grid gap-[clamp(28px,5vw,58px)] md:grid-cols-[minmax(0,1fr)_360px] md:items-end">
            <div className="max-w-[760px]">
              ...
              <p className="text-[clamp(16px,2.2vw,20px)] leading-[1.6] text-muted mt-5 mb-0 max-w-[54ch]">
                {t.sub}
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2.5">
              <div className="rounded-[16px] border border-border bg-white px-4 py-4">
                ...
                <div className="mt-1 text-[12.5px] font-medium leading-snug text-muted">{t.statsProjects}</div>
              </div>
              <div className="rounded-[16px] border border-border bg-white px-4 py-4">
                <div className="font-heading text-[clamp(24px,4vw,34px)] font-bold tracking-[-0.03em] text-ink">
                  {liveCount}
                </div>
                <div className="mt-1 text-[12.5px] font-medium leading-snug text-muted">{t.statsDomains}</div>
              </div>
              <div className="rounded-[16px] border border-border bg-white px-4 py-4">
      ...
      <section className="max-w-[1100px] mx-auto px-[22px] pb-[clamp(56px,8vw,90px)]">
        <div className="animate-reveal motion-reduce:animate-none relative overflow-hidden rounded-[26px] bg-[linear-gradient(140deg,#0C1410,#13251A)] px-[clamp(26px,5vw,58px)] py-[clamp(38px,6vw,64px)] text-center">
          ...
          <div className="relative">
            <h2 className="mx-auto m-0 max-w-[20ch] font-heading text-[clamp(26px,4.2vw,44px)] font-bold leading-[1.08] tracking-[-0.03em] text-white">
              {t.ctaTitle}
            </h2>
            <p className="mx-auto mb-0 mt-[18px] max-w-[52ch] text-[clamp(15px,2.2vw,18px)] leading-[1.6] text-text-dark-1">
              {t.ctaSub}
            </p>
